...
export function createMockContext(): RouteContext {
  ...
  const ctx: RouteContext = {
    ...
    chromeImporter: {
      ...
      importIdentities: vi.fn().mockResolvedValue({ ok: true, identities: [], cdpAvailable: false }),
    ...
    sessionManager: {
      ...
      get: vi.fn().mockReturnValue(null),
      getOrCreate: vi.fn().mockImplementation((name: string) => ({
        session: { name, partition: `persist:session-${name}`, createdAt: Date.now(), isDefault: false },
        created: true,
      })),
      electronSession: vi.fn().mockReturnValue({ cookies: { set: vi.fn().mockResolvedValue(undefined) } }),
      setActive: vi.fn(),
